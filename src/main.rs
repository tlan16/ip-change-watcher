...
use database::entities::{public_ips, prelude::PublicIps};
use sea_orm::{ActiveValue, Database, DatabaseConnection, EntityTrait, QueryOrder};
use tokio::join;
...
async fn get_last_public_ip(db: &DatabaseConnection) -> Option<public_ips::Model> {
  PublicIps::find().order_by_desc(public_ips::Column::CreatedAt).one(db).await.unwrap()
}
